...
DATASET_NAMES = (
   'iemocap_pretrain','meld_pretrain','mosi_pretrain','mosei_pretrain','emoji_pretrain','emory_pretrain','daily_pretrain','sst_pretrain','imdb_pretrain','amazon_pretrain','emowoz_pretrain','sarc_pretrain',
    ...
    'sarc_few_train','emoji_few_train','iemocap_pretrain',
...
class PseudoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self._data_id=='iemocap':    
            output['labels']=self._dataset[index]['pseudo_labels'][0]
        elif self._data_id=='meld':
            output['labels']=self._dataset[index]['pseudo_labels'][1]
        else:
            output['labels']=self._dataset[index]['pseudo_labels'][2]
...
def main(rank, args):
   ...
   ##test dataset
    ...
    if 'iemocap_test' in args.dataset:
        val_dataset_list.append(PseudoDataset(
            args.dataset['iemocap_test'],data_id='meld',use_text=True,use_image=True,use_audio=True))
    if 'meld_test' in args.dataset:
        val_dataset_list.append(PseudoDataset(
            args.dataset['meld_test'],data_id='iemocap',use_text=True,use_image=True,use_audio=True))
